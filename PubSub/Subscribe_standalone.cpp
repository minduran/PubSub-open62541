        ...
        for(int i = 0; i < dsm->data.keyFrameData.fieldCount; i++) {
            ...
            if(currentType == &UA_TYPES[UA_TYPES_BYTE]) {
            	...
            	if (i+1 < dsm->data.keyFrameData.fieldCount && &UA_TYPES[UA_TYPES_BYTE]  == dsm->data.keyFrameData.dataSetFields[i+1].value.type) {
					char *msg = (char*)malloc(sizeof(char) * (dsm->data.keyFrameData.fieldCount + 1));
					msg[0] = value;
					UA_Byte it = 1;
					i++;
...
int main(int argc, char **argv) {
    ...
    char connectionName[] = {"UADP Connection 1"};
    connectionConfig.name = UA_STRING(connectionName);

    char transportProfile[] = {"http://opcfoundation.org/UA-Profile/Transport/pubsub-udp-uadp"};
    connectionConfig.transportProfileUri = UA_STRING(transportProfile);
    connectionConfig.enabled = UA_TRUE;

    char url[] = {"opc.udp://224.0.0.22:4840/"};
    UA_NetworkAddressUrlDataType networkAddressUrl = {UA_STRING_NULL , UA_STRING(url)};
